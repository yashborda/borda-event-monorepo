...
export function DeleteBillDialog({
  ...
  const onSubmit = async (data: IFormData) => {
    try {
      await apiFetch(`/api/admin/bills/${billId}`, {
        method: 'DELETE',
  ...
  return (
    <Dialog
      ...
      title="Delete Bill"
      description={`Soft delete bill "${billNo}". It will be hidden by default; use the "Show Deleted" toggle to restore.`}
      actions={[
        {
          label: 'Cancel',
    ...
    >
      <form id="delete-bill-form" onSubmit={handleSubmit(onSubmit)} noValidate>
        <div className="pb-2">
          <Textarea
            id="reason"
            label="Reason for Deletion"
            placeholder="Enter a reason for deleting this bill…"
            required
            disabled={isSubmitting}
