...
import type { IWebsiteUser } from '@pkg/types'
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    channel.current = new BroadcastChannel('website_auth')
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        const me = await apiFetch<IWebsiteUser>('/api/website/auth/me')
        setUser(me)
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      const data = await apiFetch<{ accessToken: string; user: IWebsiteUser }>(
  ...
  const logout = useCallback(async () => {
    try {
      await apiFetch('/api/website/auth/logout', {
        method: 'POST',
        credentials: 'include',
      })
    } catch {
  ...
  const requestMagicLink = useCallback(async (email: string) => {
    await apiFetch('/api/website/auth/magic-link', {
      method: 'POST',
  ...
  const forgotPassword = useCallback(async (email: string) => {
    await apiFetch('/api/website/auth/forgot-password', {
      method: 'POST',
      body: JSON.stringify({ email }),
    })
  }, [])
  ...
  const resetPassword = useCallback(
    async (token: string, newPassword: string) => {
      await apiFetch('/api/website/auth/reset-password', {
        method: 'POST',
  ...
  const updateProfile = useCallback(
    async (data: { fullName?: string; avatarUrl?: string }) => {
      const updated = await apiFetch<IWebsiteUser>('/api/website/auth/me', {
        method: 'PATCH',
  ...
  const changePassword = useCallback(
    async (currentPassword: string, newPassword: string) => {
      await apiFetch('/api/website/auth/me/password', {
        method: 'PATCH',
  ...
  const setTokenFromCallback = useCallback(
    (token: string) => {
      ...
      apiFetch<IWebsiteUser>('/api/website/auth/me')
        .then((me) => {
          setUser(me)
          setStatus('authenticated')
        })
